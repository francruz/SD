...
#Abrimos el fichero y lo guardamos en la variable
fichero = open("pokemon.txt", "r")  
contenido = fichero.read()
#Copiamos el contenido del fichero en una lista
lista_base = contenido.split()   
fichero.close()
cont_final = 0

#Recorremos la lista de palabras n veces, siendo n el numero de palabras de esta
for i in range(len(lista_base)):    
    primera_palabra = lista_base[i]    
    lista_aux = []
    #Metemos la primera palabra a la lista auxiliar
    lista_aux.append(primera_palabra)   
    cont_aux = 1
    puntero = 0
    #Y guardamos la ultima letra de la primera palabra, para comparar
    ultima_letra = primera_palabra[len(primera_palabra)-1]  
